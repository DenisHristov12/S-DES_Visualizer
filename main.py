...
class SDESApp(ttk.Frame):
    ...
    def build_left_panel(self):
        ...
        ttk.Label(left, text="Текст (8 бита):", font=UI_FONT).grid(row=1, column=0, sticky="w")
        ...
        # Key
        ttk.Label(left, text="Ключ (10 бита):", font=UI_FONT).grid(row=3, column=0, sticky="w")
        ...
        # Buttons
        btn_encrypt = ttk.Button(left, text="Encrypt (Шифрирай)", command=self.on_encrypt)
        btn_decrypt = ttk.Button(left, text="Decrypt (Дешифрирай)", command=self.on_decrypt)
        ...
        ttk.Label(left, text=hint, font=("Segoe UI", 9)).grid(row=7, column=0, columnspan=2, sticky="w")
    ...
    def build_right_panel(self):
        ...
        lbl_result = ttk.Label(right, textvariable=self.result_var, font=("Consolas", 13))
    ...
    def _make_tab(self, nb: ttk.Notebook, title: str):
        ...
        tree = ttk.Treeview(frame, columns=("step", "value"), show="headings", selectmode="browse")
    ...
    def clear_all_tabs(self):
        for tab in (self.tab_key, self.tab_ip, self.tab_r1, self.tab_sw, self.tab_r2, self.tab_final):
